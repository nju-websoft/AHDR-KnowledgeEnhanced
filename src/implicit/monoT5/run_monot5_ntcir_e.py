# coding=utf-8
from transformers import AutoTokenizer, AutoModelForSeq2SeqLM, BertForSequenceClassification, get_linear_schedule_with_warmup
...
def inference(model, test_examples, q_rels, eval_batch_size, max_len):
    ...
    for sample in tqdm(test_examples):
        q_id = sample['q_id']
        input_ids, input_masks, c_ids = get_input_feature_inference(sample, max_len)
        pred_scores = []
        ...
        for step in range(step_count):
            ...
            attention_mask_bt = input_masks[beg_index:end_index]
...
if __name__ == '__main__':
    ...
    config_name = f'monoT5_{args.task}'
    ...
    for epoch in range(args.epoch_num):
        ...
        for step in step_trange:
            ...
            input_ids, input_masks, target_ids = get_input_feature_train(
                batch_example, args.max_len, max_pos=args.max_train_pos, max_neg=args.max_train_neg)
            loss = model(input_ids, input_masks,  target_ids)
            loss = loss.mean()
            tr_loss += loss.item()
            nb_tr_steps += 1
    ...
    print('best test scores:', json.dumps(best_test_result['mean'], indent=2))