In order to run ANCE, first refer to the README of the original repo (which we put below) and follow the process indicated there, running scripts in `commands/process_data.sh` and `commands/process_data.sh` first, after which you can run scripts `commands/run_ann_data_gen.sh` and `commands/run_train.sh` in parallel
