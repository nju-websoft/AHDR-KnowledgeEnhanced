...
# # Passage ANCE(FirstP) 
 ...
 seq_length=512
 model_type=rdot_nll
 tokenizer_type="ance-msmarco-passage"
 base_data_dir="../data/emnlp2023_data/ntcir2ntcir/"
 preprocessed_data_dir="${base_data_dir}ann_data_${tokenizer_type}_${seq_length}/"
 job_name="OSPass512_msmarco"
 pretrained_checkpoint_dir="/home2/yourname/Code/ANCE/output/from_ms_marco/ntcir/"
 data_type=1
 warmup_steps=5000
...
--model_name_or_path /home2/yourname/Models/roberta-base/ --max_seq_length $seq_length --data_type $data_type\
"
...
python -m torch.distributed.launch --master_port 9999 --nproc_per_node=$gpu_no ../drivers/run_ann_data_gen.py --training_dir $model_dir \
...
############################################# Training ########################################
train_cmd="\
python -m torch.distributed.launch --master_port 9999 --nproc_per_node=$gpu_no ../drivers/run_ann.py --model_type $model_type \
...
--gradient_accumulation_steps $gradient_accumulation_steps  --max_steps 10000 --learning_rate $learning_rate --output_dir $model_dir \
