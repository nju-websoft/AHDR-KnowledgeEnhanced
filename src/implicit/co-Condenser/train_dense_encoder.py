...
class BiEncoderTrainer(object):
    ...
    def __init__(self, args):
        ...
        if saved_state:
            self._load_saved_state(args, saved_state)
            ...
            if validation_loss < (self.best_validation_result or validation_loss + 1):
                ...
                for file in file_list:
                    ...
                    if os.path.isfile(file_path):
                        os.remove(file_path)
                cp_name = self._save_checkpoint(scheduler, epoch, iteration, validation_loss)
    ...
    def _save_checkpoint(self, scheduler, epoch: int, offset: int, validation_loss) -> str:
        args = self.args
        model_to_save = get_model_obj(self.biencoder)
        # cp = os.path.join(args.output_dir,
        #                   args.checkpoint_file_name + '.' + str(epoch) + ('.' + str(offset) if offset > 0 else ''))
        cp = os.path.join(args.output_dir,
                          args.checkpoint_file_name + '.' + str(validation_loss))
    ...
    def _load_saved_state(self, args, saved_state: CheckpointState):
        ...
        if not args.ignore_checkpoint_optimizer and saved_state.optimizer_dict:
            logger.info('Loading saved optimizer state ...')
            self.optimizer.load_state_dict(saved_state.optimizer_dict)

        if not args.ignore_checkpoint_lr and saved_state.scheduler_dict:
...
def main():
    ...
    parser.add_argument("--ignore_checkpoint_optimizer", type=bool, default=False,
                        help="ignore checkpoint optimizer")

    parser.add_argument("--ignore_checkpoint_lr", type=bool, default=False,
                        help="ignore checkpoint lr")
